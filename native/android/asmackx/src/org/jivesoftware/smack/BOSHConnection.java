...
public class BOSHConnection extends Connection {
    ...
    public void sendPacket(Packet packet) {
    	Log.d("BOSHConnection", "sendPacket(); START " + (int)(System.currentTimeMillis() / 1000L ));
        if (!isConnected()) {
            throw new IllegalStateException("Not connected to server.");
        }
        if (packet == null) {
        ...
        Log.d("BOSHConnection", "!done; BEFORE " + (int)(System.currentTimeMillis() / 1000L ));
        if (!done) {
            // Invoke interceptors for the new packet that is about to be sent.
            // Interceptors
            // may modify the content of the packet.
        	Log.d("BOSHConnection", "firePacketInterceptors(packet); BEFORE " + (int)(System.currentTimeMillis() / 1000L ));
            firePacketInterceptors(packet);
            Log.d("BOSHConnection", "firePacketInterceptors(packet); AFTER " + (int)(System.currentTimeMillis() / 1000L ));

            try {
            	Log.d("BOSHConnection", "send(ComposableBody.builder().setPayloadXML(packet.toXML()).build()); BEFORE " + (int)(System.currentTimeMillis() / 1000L ));
                send(ComposableBody.builder().setPayloadXML(packet.toXML())
                        .build());
               	Log.d("BOSHConnection", "send(ComposableBody.builder().setPayloadXML(packet.toXML()).build()); AFTER " + (int)(System.currentTimeMillis() / 1000L ));
            ...
            // thread so it's expected that listeners are fast.
           	Log.d("BOSHConnection", "firePacketSendingListeners(packet); BEFORE " + (int)(System.currentTimeMillis() / 1000L ));
            firePacketSendingListeners(packet);
           	Log.d("BOSHConnection", "firePacketSendingListeners(packet); AFTER " + (int)(System.currentTimeMillis() / 1000L ));

        }
        Log.d("BOSHConnection", "!done; AFTER " + (int)(System.currentTimeMillis() / 1000L ));
    }
    ...
    protected void send(ComposableBody body) throws BOSHException {
        ...
        if (sessionID != null) {
       	...
       	Log.d("BOSHConnection", "client.send(body); BEFORE " + (int)(System.currentTimeMillis() / 1000L ));
        client.send(body);
       	Log.d("BOSHConnection", "client.send(body); AFTER " + (int)(System.currentTimeMillis() / 1000L ));
